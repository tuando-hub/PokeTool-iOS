...
function redact(value){if(Array.isArray(value))return value.map(redact);if(!value||typeof value!=="object")return value;const out={};Object.keys(value).forEach(key=>{out[key]=/password|pass|otp|token|cookie|authorization|card|cvv|cvc|security|expiration|acs|creq|pareq|jwt/i.test(key)?"<redacted>":redact(value[key]);});return out;}
