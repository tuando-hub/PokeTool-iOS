...
final class CompactRow: UIControl {
    init(title: String, detail: String, icon: String, tint: UIColor = AppTheme.accent) { super.init(frame: .zero); backgroundColor = AppTheme.card; layer.cornerRadius = 12; let image = UIImageView(image: UIImage(systemName: icon)); image.tintColor = tint; let t = UILabel(); t.text = title; t.font = .preferredFont(forTextStyle: .callout); let d = UILabel(); d.text = detail; d.textColor = AppTheme.muted; d.font = .preferredFont(forTextStyle: .caption1); let s = UIStackView(arrangedSubviews: [image, t, d, UIImageView(image: UIImage(systemName: "chevron.right"))]); s.axis = .horizontal; s.spacing = 10; s.alignment = .center; s.translatesAutoresizingMaskIntoConstraints = false; addSubview(s); NSLayoutConstraint.activate([s.topAnchor.constraint(equalTo: topAnchor, constant: 12), s.leadingAnchor.constraint(equalTo: leadingAnchor, constant: 12), s.trailingAnchor.constraint(equalTo: trailingAnchor, constant: -12), s.bottomAnchor.constraint(equalTo: bottomAnchor, constant: -12)]); setAccessibilityLabel(title) }
    @available(*, unavailable) required init?(coder: NSCoder) { fatalError("Storyboard initialization is not supported") }
}
